...
import { AiBudgetPrintLayout } from "@/components/dashboard/budget-print-layout";
import type { Company } from "@/components/dashboard/company/company-section";
...
function BulkMoveDialog({ open, onOpenChange, onMove, chapters }: { open: boolean, onOpenChange: (open: boolean) => void, onMove: (targetChapterName: string) => void, chapters: any[] }) {
  const form = useForm<z.infer<typeof bulkMoveSchema>>({
    resolver: zodResolver(bulkMoveSchema),
  });

  const handleSubmit = (values: z.infer<typeof bulkMoveSchema>) => {
    onMove(values.targetChapterName);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Mover Partidas Seleccionadas</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Mover a capítulo</FormLabel>
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue placeholder="Elige un capítulo de destino..." />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      {chapters.map((c) => (
            ...
            <DialogFooter>
              ...
              <Button type="submit">Mover Partidas</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
...
function AiBudgetCard({ 
    ...
    const [selectedPartidas, setSelectedPartidas] = useState<Record<string, boolean>>({});
    ...
    const toggleChapter = (chapterName: string) => {
        setOpenChapters(prev => ({...prev, [chapterName]: !prev[chapterName]}));
    }

    const handleSelectPartida = (partidaId: string, isSelected: boolean) => {
      setSelectedPartidas(prev => ({ ...prev, [partidaId]: isSelected }));
    };

    const handleSelectChapterPartidas = (chapterName: string, isSelected: boolean) => {
      const chapter = budget.breakdown.capitulos.find(c => c.nombre === chapterName);
      if (!chapter) return;
      
      const newSelected = { ...selectedPartidas };
      chapter.partidas.forEach((p, index) => {
        const partidaId = `${chapterName}---${p.descripcion}---${index}`;
        newSelected[partidaId] = isSelected;
      });
      setSelectedPartidas(newSelected);
    };
    
    const selectedPartidaIds = Object.entries(selectedPartidas).filter(([,isSelected]) => isSelected).map(([id]) => id);

    const handleBulkDeleteClick = () => {
        onBulkDelete(selectedPartidaIds);
        setSelectedPartidas({});
        setHasChanges(true);
    };

    const handleBulkMoveSubmit = (targetChapterName: string) => {
        onBulkMove(selectedPartidaIds, targetChapterName);
        setSelectedPartidas({});
        ...
        <AccordionItem value={budget.id} className="border-none">
            ...
            <Card key={budget.id} className="flex flex-col">
                ...
                <AccordionContent>
                    <div className="flex-grow space-y-4 px-6 pb-6">
                            <div className="space-y-2">
                                {budget.breakdown.capitulos.map((capitulo, index) => {
                                    const isChapterOpen = openChapters[capitulo.nombre] ?? true;
                                    const allPartidasInChapter = capitulo.partidas.map((p, i) => `${capitulo.nombre}---${p.descripcion}---${i}`);
                                    const selectedInChapterCount = allPartidasInChapter.filter(id => selectedPartidas[id]).length;
                                    const areAllSelected = allPartidasInChapter.length > 0 && selectedInChapterCount === allPartidasInChapter.length;

                                    return (
                                    <div key={`${budget.id}-${capitulo.nombre}-${index}`} className="rounded-md border">
                                        <div className="flex items-center gap-2 bg-muted/50 p-2">
                                            <button onClick={() => toggleChapter(capitulo.nombre)} className="p-1">
                                                <ChevronDown className={cn("h-5 w-5 transition-transform", isChapterOpen && "rotate-180")} />
                                            </button>
                    ...
                    <CardFooter className="justify-between bg-secondary/80 p-4 mt-auto">
                        {selectedPartidaIds.length > 0 ? (
                           <div className="flex items-center gap-4">
                                <span className="text-sm font-semibold">{selectedPartidaIds.length} partidas seleccionadas</span>
                                <Button size="sm" onClick={() => setBulkMoveOpen(true)}><Move className="mr-2 h-4 w-4" /> Mover</Button>
                                 <AlertDialog>
                                    <AlertDialogTrigger asChild>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            <AlertDialogTitle>¿Eliminar {selectedPartidaIds.length} partidas?</AlertDialogTitle>
                                            <AlertDialogDescription>Esta acción no se puede deshacer.</AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
                                            <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                            <AlertDialogAction onClick={handleBulkDeleteClick}>Sí, eliminar</AlertDialogAction>
                                        </AlertDialogFooter>
                                    </AlertDialogContent>
                                </AlertDialog>
                           </div>
...
export function AiBudgetsSection({ 
    ...
    const handleDragEnd = (result: DropResult) => {
        ...
        const budgetId = draggableId.split('---')[0];
