...
function ContactForm({ contact, onSubmit, open, onOpenChange }: { contact?: Contact, onSubmit: (values: any) => void, open: boolean, onOpenChange: (open: boolean) => void }) {
  const form = useForm<z.infer<typeof contactSchema>>({
    ...
    defaultValues: contact || { name: "", phone: "", email: "", called: false, status: 'Pendiente' },
  });
  ...
  useEffect(() => {
    if (open) {
      if (contact) {
        ...
        form.reset({ name: "", phone: "", email: "", called: false, status: 'Pendiente' });
      }
    }
  }, [contact, open, form]);
                              ...
                              <TableCell className="text-right sticky right-0 bg-card">
                               <AlertDialog>
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent>
                                      <DropdownMenuItem onSelect={() => setPromotingForm(sub)}><UserPlus className="mr-2"/>Promover a Proyecto</DropdownMenuItem>
    ...
    <Card>
      <AlertDialog open={!!promotingContact} onOpenChange={(open) => !open && setPromotingContact(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Promover Contacto a Proyecto</AlertDialogTitle>
            <AlertDialogDescription>
              ¿A qué sección quieres añadir a "{promotingContact?.name}"? Se creará una nueva entrada con su nombre, email y teléfono.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="grid grid-cols-1 sm:grid-cols-2 gap-2">
             <Button variant="outline" onClick={() => handlePromote('reformas')}>Añadir a Reformas</Button>
      ...
      <CardHeader>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle>Contactos Manuales</CardTitle>
            <CardDescription>Añade y gestiona contactos que no provienen de formularios.</CardDescription>
          </div>
          <Button onClick={() => setAddDialogOpen(true)}><Plus className="mr-2 h-4 w-4" />Añadir Contacto</Button>
        </div>
      </CardHeader>
      <CardContent>
        <div className="rounded-md border">
          <Table>
            <TableHeader><TableRow>
                <TableHead>Nombre</TableHead><TableHead>Teléfono</TableHead><TableHead>Email</TableHead>
                  ...
                  <TableCell className="text-right">
                    <AlertDialog>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreHorizontal /></Button></DropdownMenuTrigger>
                        <DropdownMenuContent>
                          <DropdownMenuItem onSelect={() => setPromotingContact(contact)}><UserPlus className="mr-2"/>Promover a Proyecto</DropdownMenuItem>
                          <DropdownMenuItem onSelect={() => handleEdit(contact)}><Pencil className="mr-2"/>Editar</DropdownMenuItem>
                          <AlertDialogTrigger asChild><DropdownMenuItem className="text-destructive"><Trash2 className="mr-2"/>Eliminar</DropdownMenuItem></AlertDialogTrigger>
                        </DropdownMenuContent>
                      </DropdownMenu>
                      <AlertDialogContent>
                          ...
                          <AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => onDeleteContact(contact.id)}>Eliminar</AlertDialogAction></AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </TableCell>
                </TableRow>
