...
function ChecklistForm({ checklist, onSubmit, open, onOpenChange }: { checklist?: JordanItem, onSubmit: (values: any) => void, open: boolean, onOpenChange: (open: boolean) => void }) {
...
function NoteForm({ note, onSubmit, open, onOpenChange }: { note?: JordanItem, onSubmit: (values: any) => void, open: boolean, onOpenChange: (open: boolean) => void }) {
...
// --- Main Component ---
export function JordanSectionCard({
    items,
    onAddItem,
    onUpdateItem,
    onDeleteItem,
    ...
    team: TeamMember[],
    onPassChecklist: (item: JordanItem, targetUserId: string, annotations: string) => void;
}) {
    const [activeForm, setActiveForm] = useState<'note' | 'checklist' | null>(null);
    const [editingItem, setEditingItem] = useState<JordanItem | undefined>(undefined);
    const [viewingNote, setViewingNote] = useState<JordanItem | null>(null);
    const [passingItem, setPassingItem] = useState<JordanItem | null>(null);


    const handleEdit = (item: JordanItem) => {
        setEditingItem(item);
    ...
    const handleToggleItem = (checklist: JordanItem, itemIndex: number) => {
        if (!checklist.items) return;
        const newItems = [...checklist.items];
        newItems[itemIndex] = { ...newItems[itemIndex], completed: !newItems[itemIndex].completed };
        onUpdateItem({ ...checklist, items: newItems });
    };
    
    const handleToggleCompleted = (note: JordanItem) => {
        onUpdateItem({ ...note, completed: !note.completed });
    };

    const handlePassClick = (item: JordanItem) => {
        setPassingItem(item);
    };
    ...
    const simpleNotes = items.filter(n => n.type === 'note');
    const checklists = items.filter(n => n.type === 'checklist' || !n.type); // backwards compatibility
    ...
    return (
        <Card>
            ...
            <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <CardTitle className="flex items-center gap-2"><ListChecks /> Apuntes y Tareas de Jordan</CardTitle>
                                        ...
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                                ...
                                                <AlertDialogFooter>
                                                    ...
                                                    <AlertDialogAction onClick={() => onDeleteItem(note.id)}>Eliminar</AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                                        </AlertDialog>
                                    </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <AlertDialog>
                                                ...
                                                <AlertDialogContent>
                                                    ...
                                                    <AlertDialogFooter>
                                                        <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                                        <AlertDialogAction onClick={() => onDeleteItem(checklist.id)}>Eliminar</AlertDialogAction>
                                                    </AlertDialogFooter>
                                                </AlertDialogContent>
                                            </AlertDialog>
                                        </div>
