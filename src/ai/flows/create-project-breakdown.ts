...
const ProjectBreakdownSchema = z.object({
  capitulos: z.array(z.object({
    ...
    partidas: z.array(z.object({
      ...
      precioUnitario: z.string().optional().describe("El precio unitario de la partida. Extrae SÓLO el valor numérico. Ej: '12.50' o '12,50'. Si no hay precio, deja el campo vacío.").refine(
        (val) => val === undefined || val === null || /^[0-9.,]+$/.test(val), {
        message: "El precio unitario debe ser un valor numérico o nulo."
      }),
    }))
...
const projectBreakdownPrompt = ai.definePrompt({
  ...
  prompt: `
    ...
    4.  Cada partida debe tener una descripción clara.
    5.  Extrae la 'medicion' (cantidad) y 'unidad' de medida si se especifican. Si no, puedes poner 'Según proyecto', 'N/A' o similar.
    6.  **MUY IMPORTANTE**: Para el campo 'precioUnitario', extrae ÚNICAMENTE el valor numérico del precio (ej: '12.50', '12,50'). NO incluyas el símbolo del euro (€), texto como '/ud' o '/m2'. Si no se especifica un precio numérico claro, deja el campo 'precioUnitario' vacío o nulo. Este campo es el más importante.
    7.  Organiza toda la información en la estructura JSON solicitada. No inventes información que no esté en el documento. Sé preciso y cíñete al contenido del PDF.
    8.  IMPORTANTE: Ignora cualquier partida que esté vacía o no contenga una descripción clara.
